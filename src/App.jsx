...
// --- EXPIRATION TIMERS ---
const getExpiry = (createdAt, daysCount) => {
  if (!createdAt) return 'Unknown';
  const expiryTime = createdAt + (daysCount * 24 * 60 * 60 * 1000); 
...
const getExpiry7 = (createdAt) => getExpiry(createdAt, 7);
const getExpiry30 = (createdAt) => getExpiry(createdAt, 30);

// STRONGLY COMPRESS TO AVOID FIRESTORE 1MB LIMIT CATASTROPHE
const compressAndConvertImage = (file, maxDim = 800) => {
        ...
        <div className="w-full bg-slate-900 shadow-md relative rounded-t-xl overflow-hidden flex justify-center border-b-4 border-[#C5A03A]">
          {embed.type === 'youtube' ? (
             ...
             <div className="w-full py-16 px-4 flex flex-col items-center justify-center text-center text-white space-y-4">
                ...
                <p className="text-xs text-slate-400 max-w-sm leading-relaxed pb-2">Due to Google security protocols, this asset must be viewed directly. Click below to safely open it in a new window.</p>
        ...
        <div className="fixed inset-0 z-[99999] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <form onSubmit={startUpload} className="bg-white border-2 border-[#EADFC9] p-6 rounded-2xl w-full max-w-sm space-y-4 font-sans shadow-skeuo-lg animate-fadeIn">
            <div className="border-b pb-2 mb-2">
              ...
              <p className="text-[10px] text-slate-500 mt-1">Due to database limits, direct gallery uploads are disabled. Please paste a link to Google Drive, Google Photos, YouTube, or an MP4.</p>
            </div>
            <div>
...
function PostsWorkspace({ posts, userProfile, showToast, pushNotification, isAdmin, onInspectUser }) {
  ...
  const publishPost = async (e) => {
    ...
    } catch (err) { showToast('Image too large or compression failed.', 'warning'); } finally { setPublishing(false); }
  };
